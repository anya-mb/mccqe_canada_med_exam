...
def test_committed_wave_is_plan_bound_and_progress_is_derived() -> None:
    ...
    assert progress["SOURCE_PACKETS_READY"] == 90
    assert progress["SOURCE_PACKETS_PENDING"] == 1434
    assert progress["RESEARCH_BATCHES_COMPLETE"] == 10
    assert progress["RESEARCH_BATCHES_PENDING"] == 149
