...
def test_current_repository_reconstructs_valid_source_research_state() -> None:
    ...
    assert state.progress["SOURCE_PACKETS_READY"] == 90
    assert state.progress["SOURCE_PACKETS_PENDING"] == 1434
    assert state.progress["RESEARCH_BATCHES_COMPLETE"] == 10
    assert state.progress["RESEARCH_BATCHES_PENDING"] == 149
    assert state.progress["AUTHORITATIVE_SOURCE_RECORDS"] == 116
    assert len(state.registry["documents"]) == 75
    assert len(state.registry["input_populations"]) == 10
    assert len(state.readiness["jobs"]) == 220
    assert state.readiness["summary"] == {
        "GENERATION_JOBS_TOTAL": 220,
        "GENERATION_JOBS_SOURCE_READY": 11,
        "GENERATION_JOBS_PENDING": 209,
        "GENERATION_JOBS_BLOCKED": 0,
        "QUESTION_SLOTS_TOTAL": 6086,
        "QUESTION_SLOTS_SOURCE_READY": 330,
    }
    assert len(state.queue["jobs"]) == 11
    assert state.queue["summary"] == {
        "GENERATION_QUEUE_JOBS": 11,
        "QUESTION_SLOTS_QUEUED": 330,
    }
