...
- Audited coordinator input commit: `bb2d0cf786a7dfff5a8106c94447cc61fc9b5ba8`.
...
- **First production pilot independently verified: `PILOT_V1_INDEPENDENT_VERIFICATION=COMPLETE`, 11 of 12 accepted (2026-09-13)**, in a fresh verifier session that never acted as author. `QGEN_BRANCH=codex/generalized-qgen-onboarding`, `MAIN_WEB_BRANCH=main` left at `1262883` and untouched; **no merge, no push, no web export**. Two scoped commits: authoring freeze `cf928f9657380c5f73183d0c19500a70808737d7` (the AUTHOR state, created before any verification) and the verification commit that carries this section. Milestone `reports/qgen_pilot_v1_independent_verification_milestone.json` (sha256 `07559b6b989df71b38e68604ed0e0d7816fcc7027859e931952a5708ee7830aa`).
- **Phase-0 hash gate PASS.** Registry V3 `57a12e09…`, pilot manifest `5a20b814…`, handoff `91050b09…`, package artifact `2dae1027…` and the package schema file all reproduce their declared digests; all 12 packages reproduce their own `content_sha256`; all 12 opportunity ids resolve in both Registry V3 and the 36-row pilot manifest; the ledger was empty, so no item arrived pre-accepted. Each Stage-1 packet was checked to equal the deterministic `blind_projection` of its package and to carry none of the nine author-material fields — blindness was verified, not assumed.
- **Stage 1 was frozen before any author material was opened.** `pilot_stage_1_blind_results.json` (sha256 `b652336a1b72da03dd1dd6d5ac388e8ddf7523a85dba0c9f1df87dc44e73e556`), 12/12 solved at HIGH confidence, `STAGE1_KEY_MATCHES=12`, `STAGE1_KEY_MISMATCHES=0`, `STAGE1_AMBIGUOUS=0`, `STAGE1_UNANSWERABLE=0`, `STAGE1_SECOND_KEY_FLAGS=0`. **Blind agreement of 1.00 was explicitly not treated as acceptance**; every item still had to pass Stage 2.
- **Stage 2 rejected one item that the verifier had answered identically to the author.** `FINAL_VERDICTS={VERIFIED_ACCEPT: 11, REJECT_UNSUPPORTED_RATIONALE: 1}`. `QPILOT-V1-OBGYN-02` (routine first-trimester prenatal panel) fails because its single evidence fact asserts only that the initial assessment includes some specified baseline blood and infection screening and names no constituent test and no gestational timing, so nothing in the package discriminates the keyed blood group and antibody screen from the GBS, glucose-challenge and fetal-fibronectin distractors; the author rationale supplies a six-test panel and specific screening windows that no cited source states. The keyed option is very probably correct as Canadian practice — it is rejected on **entailment**, under the standing rule against reconstructing missing evidence from medical knowledge. Queue `pilot_v1_rejection_revision_queue.json` (sha256 `df5fecc1195342078fb22ab694fcb56a2ca9cb59f6512f1563cfe1979050a4e5`) names the remedy as the PHAC prenatal Appendix B laboratory table already sitting in the item's own source locator.
- **No citation was fabricated anywhere in the pilot.** All 27 cited fact ids resolve in the 373-fact evidence index and every underlying source row carries a real organization, title, URL and retrieval date; the readiness layer was retrieved 2026-09-08, five days before verification. Accepted items: `pilot_v1_verified_accept_manifest.json` (sha256 `b2caa384f6675a2ec78d7a19c1b112c763960c3c17191d3dfb51d3bc219c112f`), 11 items each pinned to question version, package sha, Registry V3 opportunity id, Stage-1 sha, Stage-2 sha and ledger entry sha. Claim-level audit in `pilot_stage_2_claim_audits.json` (sha256 `0858ccb0779ba33cb9c55e24e5bce584d10283f02a6a247a058dc682d0ed4371`).
- **Ledger chain PASS**, 12 entries, `production_item_verification_ledger_v1.json` content sha256 `4fe8b68e5073f6c35d291a1b8e604fa0d557171cbefec296e203033198373dc8`, 7/7 mutation detections still green. `AUTHOR_CONTENT_MODIFIED_BY_VERIFIER=NO` — verified by diffing the authored items, packages, blind packets, handoff and the whole Registry V3 directory against the freeze commit.
- **Two tests pinned literal digests of values designed to change, and both were repaired rather than worked around.** The dry-run manifest test pinned the raw file digest of the **append-only** production ledger, so any legitimate verified item broke it; it now asserts the ledger's self-describing content hash and clean chain. The safe-resume test pinned a literal `CURRENT_REPOSITORY_HEAD`, so every commit broke it; it now asserts the reported head is the real head and still descends from the frozen historical baseline.
- **Three infrastructure findings recorded for a later session, none of which is a verifier's to fix.** `build_evidence_index` mis-slots provenance in two layers (readiness puts a document title in `organization` and a retrieval date in `date_or_version`; holdout puts a title in `date_or_version`), so package `canadian_source_refs` display wrong field contents while the underlying rows are correct. That same field also legitimately holds non-Canadian sources — ACC/AHA and ACG back `QPILOT-V1-MED-01` — so its name overstates its contents. And verifier artifacts must not carry an `evidence` token in their filename under `research/qgen`, because the feature-evidence reuse baseline rglobs that token and would silently count verification records as evidence supply; that mistake produced this session's only regression and was fixed by renaming, not by re-pinning counts.
- **`EVIDENCE_READY_UNUSED_OPPORTUNITIES=157`** of the 445 production-eligible rows (`pilot_v1_unused_opportunity_supply_analysis.json`, sha256 `6b3fece82f625a51235b0c3e84725a616b7387b5125f2da39226800f35f05267`), by deterministic join on holdout study units and on READY source packets matched by discipline and MCC objective; 146 sit outside the Pilot V1 manifest, skewed hard to MED 125 and OBGYN 19. **This is an availability signal, not entailment.** Pilot V1 is the only calibration we have and it converted 12 of 36 (33%), so treat 157 as an upper bound of roughly 50 authorable items. `NEXT_AUTHORING_SUPPLY_STRATEGY=BOTH`: select an evidence-aware extension manifest from these candidates *and* run targeted source research, because neither alone reaches a 20-item gate in PED, PSY, PHELO or SURG.
- Focused tests **132 passed / 0 failed** (`test_independent_item_verification.py`, `test_production_pilot_authoring.py`, `test_registry_v3_canonicalization.py`, `test_curriculum_opportunity_registry_v2.py`). Full suite rerun after the artifact rename and the copyright repair: **2,152 passed / 1 failed**, the one failure being the known unrelated `test_source_research_coordinator.py::test_discovery_classifies_dirty_worktree_retry_and_committed_branch_awaiting_integration`; `ACTUAL_NEW_REGRESSIONS=0`. Copyright audit **PASS** (`reports/qgen_pilot_v1_independent_verification_copyright_audit.json`, sha256 `65bf7b8030805b7ffae74011f27d896c9545156c0c9023596b0fe5b58e0eb28b`): the detector first caught the verifier's *own* 21-word quotation of a source statement and three claim restatements of 13-19 words, all paraphrased before freeze, leaving a longest run of 11 words. `CLAUDE.md` unchanged.
- `WEB_EXPORT_AUTHORIZED=YES_FOR_VERIFIED_ACCEPT_ONLY` but `WEB_EXPORT_PERFORMED=NO`; the 11 accepted items are the first genuinely independently verified production questions in this repository and must not reach public content until the controlled merge runs. `NEXT_DOMINANT_BOTTLENECK=DECISION_LEVEL_SOURCE_EVIDENCE_COVERAGE_OF_PRODUCTION_ELIGIBLE_OPPORTUNITIES` — authoring capacity and verification capacity are both demonstrated; evidence supply is what limits the bank. `QGEN_NEXT_STEP=CONTROLLED_QGEN_TO_MAIN_MERGE_AND_VERIFIED_CONTENT_EXPORT`, which must reconcile both branch histories, run QGEN and web regressions, export only the 11 `VERIFIED_ACCEPT` items into the web content schema, preserve the translation and Firebase architecture, and make it structurally impossible for an unverified question to enter public content.
